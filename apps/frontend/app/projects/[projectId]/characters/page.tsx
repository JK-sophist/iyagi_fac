...
const fieldMeta: Array<{ key: keyof CharacterForm; label: string; placeholder: string; helper: string; multiline?: boolean }> = [
  { key: 'name', label: '이름', placeholder: '예) 허준기', helper: '인물의 이름입니다.' },
  {
    key: 'archetype',
    label: '인물 유형',
    placeholder: '예) 냉정한 전략가, 이상주의 해커, 현실적인 생존자',
    helper: '이 인물이 어떤 타입의 사람인지 한 줄로 정리합니다.'
  },
  {
    key: 'surface_goal',
    label: '겉으로 드러난 목표',
    placeholder: '예) 조직을 지킨다, 사건을 해결한다',
    helper: '다른 인물들이 보기에도 드러나는 공식적인 목표입니다.'
  },
  {
    key: 'hidden_goal',
    label: '실제 목표',
    placeholder: '예) 권력을 잡는다, A와 접촉한다, 조직을 무너뜨린다',
    helper: '겉으로는 숨기지만 실제로 원하는 목적입니다.'
  },
  {
    key: 'short_term_goal',
    label: '지금 당장 이루려는 목표',
    placeholder: '예) 팀장의 의심을 피한다, 비밀 파일을 확보한다',
    helper: '현재 장면들에서 가장 직접적으로 이 인물을 움직이게 만드는 목표입니다.'
  },
  {
    key: 'long_term_goal',
    label: '장기 목표',
    placeholder: '예) 인간 사회를 떠난다, 지배 체제의 핵심에 올라간다',
    helper: '이야기 전체에서 끝까지 밀고 가는 큰 목표입니다.'
  },
  {
    key: 'fear_or_taboo',
    label: '두려움 또는 금기',
    placeholder: '예) 정체가 드러나는 것, 무력하게 남는 것',
    helper: '반드시 피하고 싶어하는 것 또는 넘고 싶지 않은 선입니다.'
  },
  {
    key: 'leverage',
    label: '타인을 움직이는 수단',
    placeholder: '예) 정보력, 기술력, 협박거리, 매력',
    helper: '다른 사람을 설득하거나 압박할 때 쓰는 무기입니다.'
  },
  {
    key: 'secret',
    label: '숨기고 있는 비밀',
    placeholder: '예) 이미 A와 접촉했다, 기록을 조작했다',
    helper: '아직 다른 인물들에게 들키지 않은 핵심 비밀입니다.'
  },
  {
    key: 'speaking_style_note',
    label: '말투 메모',
    placeholder: '예) 짧고 차갑게 말함, 비꼬는 어조, 감정 숨김',
    helper: '대사 생성 시 참고할 말투와 표현 습관입니다.'
  },
  {
    key: 'writer_note',
    label: '작가 메모',
    placeholder: '예) 초반엔 충직하지만 후반에 배신 가능성 높음',
    helper: '작가가 따로 기록해두는 참고 메모입니다.',
    multiline: true
  }
];
    ...
    <section className="space-y-4">
      ...
      <form onSubmit={createCharacter} className="rounded-2xl border border-border bg-card p-4 shadow-soft space-y-4">
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          ...
          <button type="button" className="rounded-xl bg-panel px-3 py-2" onClick={updateCharacter} disabled={!selectedId}>
            선택 캐릭터 수정
          </button>
        </div>
      </form>
